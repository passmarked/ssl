...
const testFunc      = require('../lib/checks/sslv2');
...
describe('sslv2', function() {
  ...
  it('Should not return a error if SSL2 does not connect', function(done) {

    payload = passmarked.createPayload({

        url: 'https://example.com/',
        testingStdout: fs.readFileSync('./samples/ssl2.good.txt').toString()
  ...
  it('Should return a error if the SSL3 was able to connect', function(done) {
    ...
    payload = passmarked.createPayload({
        ...
        url: 'https://example.com/',
        testingStdout: fs.readFileSync('./samples/ssl2.bad.txt').toString()
    ...
    testFunc(payload, {
      ...
      // check for a error
      var rule = _.find(rules, function(rule) { return rule.key == 'ssl2' });
