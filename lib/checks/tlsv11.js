...
module.exports = exports = function(payload, options, fn) {
  ...
  ssl.exec(args.join(' '), function(err, stdout, stderr) {
    ...
    if(err) {
      ...
      // output to stderr
      payload.debug('checks', 'Something went wrong while checking for TLSv1.1 support');
    ...
    payload.addRule({
      ...
      type:           'error',
      key:            'tls1.1',
      message:        'TLS v1.1 not supported'

    }, {

        display:      'code',
        message:      'The server $ does not have TLS v1.1 enabled',
        code:         build,
        identifiers:  [ address ]
